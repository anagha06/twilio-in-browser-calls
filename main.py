...
def call():
    print("Received call request with form data:")
    p.pprint(request.form)
    response = VoiceResponse()
    dial = Dial(callerId=twilio_number)

    if 'To' in request.form and request.form['To'] != twilio_number:
        print(f'Making outbound call to: {request.form["To"]}')
        dial.number(request.form['To'])
        response.append(dial)
        print(f'Generated TwiML: {response}')
        return str(response)
    else:
        print('No valid "To" number found in request')
